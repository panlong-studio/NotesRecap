...
> [!NOTE]
>
> | **结构体**          | **对应概念**                  | **对应概念的存储位置** | **获取函数**         | **核心来源**          | **包含的核心信息**                                           |
> | ------------------- | ----------------------------- | ---------------------- | -------------------- | --------------------- | ------------------------------------------------------------ |
> | **`struct dirent`** | **目录项**                    | 磁盘（目录文件内）     | `readdir()`          | **目录的数据块**      | 文件名 + inode 编号                                          |
> | **`struct stat`**   | **inode (元数据)**            | 磁盘（inode 区）       | `stat()`             | **文件的 Inode 节点** | 权限、大小、物理块指针                                       |
> | **`FILE*`  **       | **文件打开对象**`struct file` | **内核内存（内核区）** | `fopen()` / `open()` | 内核进程控制块 (PCB)  | 当前读写位置 (offset)、状态标志、**引用计数**、指向 Inode 的指针 |
